...
| Year	| Total Annual Coinbase Reward (MN)	| The Coinbase reward for each consensus round (MN)  | Consensus Round (From-To) |
| :---: | :---: | :---: | :---: |
...
<h2><b>1.2 Method to calculate the unit transaction fee</b></h2>
...
<p><li>The address that directly references the unit will be rewarded with unit reference fee (same as TrustNote 1.0)</li></p>
...
<p><li>Once the Attestion unit on the main chain is determined and stabilized, the consensus round index of the stable normal units connected to it will be the same as the consensus round index of TrustME unit.</li></p>
...
<h1><b>2. Attestation reward distribution for each round</b></h1>

<p>公证奖金分配比例</p>

<h2><b>2.1 Attestation reward distribution</b></h2>
...
<h2><b>2.2 Super Node TFans Ranking Reward</b></h2>

<p>非链上功能，由运营人员处理。</p>

<p><li>off-chain functions are handled by the operator as requires human interaction.</li></p>

<p>若发现某公证人发送双花、无效或错误单元，则该公证人会被剥夺公证权，并将其从公证人列表中删除。</p>

<p><li>If it is turned out that an Attestor has a double spent, invalid or wrong unit, the Attestor will be deprived of the Attestation on this round and it will be removed from the Attestors list of the current round.</li></p>
...
<p>Once the first TrustME unit of the new consensus round becomes stable, The public Attestor of the previous consensus round can calculate the coinbase rewards and send it to themselves and others can verify it.</p>

<p>问题2：如何避免一个公证人为了获得更多coinbase不断发公证单元？</p>

<p>Question 2: How to avoid a public Attestor from issuing more TrustME units in order to get more coinbase?</p>

<p>在计算公证人Coinbase奖励的时候，主链上同一个公证人的有效公证单元之间必须有三分之一以上的其他公证人发送的公证单元。选出下轮最后一个公证人的公证单元按double计算。</p>

<p>The solution for this is that if an Attestor have multiple TrustME units on the main chain, they should satisfy the condition as: MCI of New unit = MCI of previous unit + (1/3) * Number of Attestors. If this condition is not satisfied the new TrustME unit will not be counted in calculation of the rewards.</p>
